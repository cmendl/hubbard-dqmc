...
typedef struct
{
	double *expVu[2];		//!< exp(-lambda), exp( lambda) with lambda = acosh(exp(dt*U/2)), used for spin-up   Green's function
	double *expVd[2];		//!< exp( lambda), exp(-lambda) with lambda = acosh(exp(dt*U/2)), used for spin-down Green's function
